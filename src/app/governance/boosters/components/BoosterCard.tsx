...
function getIcon(iconName: string): ReactElement {
  ...
  return (
    <BoosterIcon
      name={randomIconName}
      className="flex-shrink-0 bg-breadgray-charcoal"
    ></BoosterIcon>
  );
}
...
const ViewButton = ({
  ...
  const verifiedButtonContent = (
    <div className="flex items-center justify-center gap-2">
      <div className="w-5 text-status-success">
        <CheckIcon />
      </div>
      <span className="font-semibold text-xl">Verified</span>
      <span className="font-normal text-base dark:text-breadpink-shaded">
        view
      </span>
    </div>
  );
  ...
  return (
    ...
    >
      {typeof content === "string" ? (
        <span className="font-semibold text-[20px]">{content}</span>
      ) : (
        content
      )}
    </button>
...
export function expiry(
  ...
  return (
    <div className="flex flex-row items-center justify-center mb-[-6px]">
      <span className={`${textColorClass} leading-none mb-[6px] mr-[6px]`}>
