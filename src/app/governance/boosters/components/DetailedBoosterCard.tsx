...
function requirement(text: String, complete: Boolean): ReactElement {
  return (
    <div className="flex flex-row gap-2 justify-start items-center">
      {complete ? (
        <span className="w-[21px] h-[21px] text-status-success ml-[3px] mt-[2px]">
          {CheckDoubleIcon()}
        </span>
      ) : (
        <span className="w-[24px] h-[24px] text-status-danger">
          {CloseIcon()}
        </span>
      )}
      <span className="text-breadgray-og-dark dark:text-breadgray-ultra-white mb-[-2px]">
        {text}
      </span>
    </div>
  );
